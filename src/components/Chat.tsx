...
import { Send, Mic, Paperclip, X, Image as ImageIcon, FileText, Loader2, Globe, Plus, Copy, Download, Check, Sparkles, Trash2, Cloud } from 'lucide-react';
    ...
    <div className="flex flex-col h-[100dvh] bg-black text-white overflow-hidden relative">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          className="absolute top-[-20%] left-[-10%] w-[500px] h-[500px] bg-indigo-900/40 rounded-full mix-blend-screen filter blur-[120px]"
        />
        <motion.div
          animate={{
            scale: [1, 1.2, 1],
          ...
          className="absolute bottom-[-20%] right-[-10%] w-[600px] h-[600px] bg-emerald-900/30 rounded-full mix-blend-screen filter blur-[120px]"
        />
      </div>

      {/* Header */}
      <header className="flex items-center justify-between p-4 z-10 bg-black/50 backdrop-blur-md border-b border-white/5">
                  ...
                  <div
                    className={`max-w-[85%] rounded-3xl p-4 ${
                      msg.role === 'user'
                        ...
                        : 'bg-zinc-900/80 border border-white/5 text-zinc-200 rounded-bl-sm backdrop-blur-md'
                    }`}
                  >
                    {msg.file && (
      ...
      {/* Input Area */}
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black via-black to-transparent z-20 pt-10">
        <motion.div 
          layout
          className="max-w-3xl mx-auto bg-zinc-900/90 backdrop-blur-xl border border-white/10 rounded-3xl p-2 shadow-2xl flex flex-col gap-2"
        >
          <AnimatePresence>
            {selectedFile && (
              <motion.div
